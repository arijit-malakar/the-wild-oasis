import type { BookingStats } from "../bookings/bookingType";
...
const SalesChart: React.FC<SalesChartProps> = ({ bookings, numDays }) => {
  ...
  const data = allDates.map((date) => ({
    ...
    totalSales: bookings.reduce((sum, booking) => {
      if (isSameDay(date, new Date(booking.created_at)))
        sum += booking.totalPrice;
      return sum;
    }, 0),
    extrasSales: bookings.reduce((sum, booking) => {
      if (isSameDay(date, new Date(booking.created_at)))
        sum += booking.extrasPrice;
      return sum;
    }, 0),
  }));
