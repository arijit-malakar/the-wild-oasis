...
const Pagination: React.FC<PaginationProps> = ({ count }) => {
  ...
  const nextPage = () => {
    // Check whether current page is the last page, else go to next page
  ...
  return (
    <StyledPagination>
      <P>
        Showing <span>{(currentPage - 1) * COUNT_PER_PAGE + 1}</span> to{" "}
        <span>
          {currentPage === pageCount ? count : currentPage * COUNT_PER_PAGE}
        </span>{" "}
